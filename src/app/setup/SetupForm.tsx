...
const ERRORS: Record<string, { en: string; es: string }> = {
  ...
  already_registered: { en: 'You already have an account. Please log in.', es: 'Ya tenés cuenta. Iniciá sesión.' },
  identity_mismatch:  { en: "We couldn't verify your details. Check your name and phone number.", es: 'No pudimos verificar tus datos. Revisá nombre y teléfono.' },
...
function SetupFormInner() {
  ...
  const isAlreadyRegistered = error === ERRORS.already_registered[locale]
        ...
        <div className="w-full max-w-sm">
          ...
          <div className="bg-slate-900 border border-slate-800 rounded-2xl shadow-2xl p-5 sm:p-8">
            ...
            <form onSubmit={handleSubmit} className="space-y-4">
              ...
              {error && (
                <div className="flex flex-col gap-2.5 text-sm text-red-400 bg-red-950/40 border border-red-800/50 rounded-lg px-3.5 py-3">
                  <span className="leading-5">{error}</span>
                  {isAlreadyRegistered && (
                    <Link
                      href="/login"
                      className="inline-flex items-center gap-1.5 font-semibold text-brand-400 hover:text-brand-300 underline underline-offset-2"
                    >
